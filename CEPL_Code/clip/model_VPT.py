...
class ResidualAttentionBlock_EVP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False, i=0, design_details=None):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0, design_details=None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_EVP(width, heads, attn_mask, True, i, design_details) if prompts_needed > i
                                            else ResidualAttentionBlock_EVP(width, heads, attn_mask, False , i, design_details)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # get patch token
        x_f = x #1*197+N_CTX*768
        x_f = x_f[:, :197, :]  # [1,197,768]
        ...
        return x , x_f, x_512
        # x:1*512, x_f:1*197*768, x_512:1*197*512
