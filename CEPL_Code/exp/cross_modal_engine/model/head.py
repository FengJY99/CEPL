...
def get_zero_shot_weights(text_dataset, num_classes, in_features, text_encoder, device="cuda"):
    with torch.no_grad():
        ...
        for label in range(num_classes):
            ...
            text_features = text_encoder(texts, eot_indices)  #1*512 什么都没做
            text_features = text_features.mean(dim=0)  # 若有多个，合为一个
...
def make_classifier_head(classifier_head,
                         ...
                         text_encoder,
                         bias=False):
    assert classifier_head in AVAI_HEADS  # linear adapter
