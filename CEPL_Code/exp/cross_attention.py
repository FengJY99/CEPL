...
class RegressorBlock(nn.Module):
    ...
    def forward(self, x_q, x_kv, pos_q, pos_k, bool_masked_pos):#要加position吗 CAE position EVIT position 正常position 看一个正常CA代码
        x, attention_map = self.cross_attn( self.norm1_q(x_q), bool_masked_pos, k=self.norm1_k(x_kv), v=self.norm1_v(x_kv) )
        x = x_q + self.drop_path(self.gamma_1_cross * x)
...
FeatureAugmentation.
'''
# patch_size=16, num_classes=1000, embed_dim=embed_dim,, depth=depth, 
# num_heads=12, mlp_ratio=4, qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
# norm_layer=partial(nn.LayerNorm, eps=1e-6), init_values=args.decoder_layer_scale_init_value, 
# num_patches=self.encoder.patch_embed.num_patches, init_std=0.02, args=args
class FeatureAugmentation(nn.Module):
    def __init__(self, patch_size=16, num_classes=8192, embed_dim=768, depth=4, 
    ...
    def forward(self, x_masked, x_unmasked, pos_embed_masked, pos_embed_unmasked, bool_masked_pos):                
        ...
        for blk in self.regressor_blocks:
            x_masked, attention_map = blk(x_masked, x_unmasked, None, None, None)
